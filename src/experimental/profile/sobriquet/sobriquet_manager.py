...
class SobriquetManager:
        ...
        try:
            ...
            if self.min_count_for_prompt > 0: 
                for sobriquet_tuple in selected_sobriquets_candidates:
                    if sobriquet_tuple[3] >= self.min_count_for_prompt: 
        ...
        if isinstance(reliable_mappings, dict) and reliable_mappings: 
            ...
            for user_id_str, sobriquet_value in reliable_mappings.items():
                ...
                try:
                    ...
                    await loop.run_in_executor(None, self.db_handler.update_group_sobriquet_count, person_id, group_id, sobriquet_value, 1)
                except Exception as e: 
        ...
        if isinstance(unreliable_mappings, dict) and unreliable_mappings: 
            ...
            for user_id_str, sobriquet_value in unreliable_mappings.items():
                ...
                try:
                    ...
                    await loop.run_in_executor(None, self.db_handler.set_group_sobriquet_value, person_id, group_id, sobriquet_value, new_count)
    ...
    async def _call_llm_for_sobriquet_analysis(
        ...
        try:
            ...
            unreliable_mappings = result.get("unreliable_mappings") 
