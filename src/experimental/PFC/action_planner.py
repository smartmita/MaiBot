...
direct_reply: 直接回复对方
send_memes: 发送一个符合当前聊天氛围或{persona_text}心情的表情包，当你觉得用表情包回应更合适，或者想要活跃气氛时选择。请注意不要滥用或者刷屏。
...
{{
    ...
    "reason": "选择该行动的原因 ",
    "emoji_query": "string" // 可选。如果行动是 'send_memes'，必须提供表情主题(填写表情包的适用场合或情感描述)；如果行动是 'direct_reply' 且你想附带表情，也在此提供表情主题，否则留空字符串。请注意不要滥用或者刷屏。
}}
...
{{
    "action": "选择的行动类型 (必须是上面列表中的一个)",
    "reason": "选择该行动的原因",
    "emoji_query": "string" // 可选。如果行动是 'send_memes'，必须提供表情主题(填写表情包的适用场合或情感描述)；如果行动是 'send_new_message' 且你想附带表情，也在此提供表情主题，否则留空字符串。请注意不要滥用或者刷屏。
}}
...
{{
    ...
    "emoji_query": "string" // 可选。如果行动是 'reply_after_wait_timeout' 且你想附带表情，在此提供表情主题，否则留空字符串。请注意不要滥用或者刷屏。
}}
