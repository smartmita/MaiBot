...
logger = get_logger("pfc_idle_manager")

class IdleManager:
    """主动聊天管理器
    负责管理所有主动聊天实例，决定何时触发主动聊天以及选择哪个用户
    """
    ...
    _last_global_trigger_time: float = time.time() - 2000  # 最后一次全局触发时间
        ...
        try:
            ...
            for stream_id, stream in private_streams.items():
                try:
                    ...
                    # 创建IdleChat实例
                    IdleChat.get_instance(stream_id, private_name)
    ...
    async def get_next_available_user(cls) -> Optional[str]:
        ...
        async with cls._global_lock:
            ...
            # 获取所有实例中的用户
            all_users = set()
            for key in IdleChat._instances:
                user = key.split(":", 1)[0]
                all_users.add(user)
...
if __name__ == "__main__":
    asyncio.run(IdleManager.initialize_all_streams())
else:
    ...
    try:
        ...
        if loop.is_running():
            # 如果事件循环正在运行，则创建任务
            asyncio.create_task(IdleManager.initialize_all_streams())
            logger.info("已在现有事件循环中启动主动聊天初始化任务")
        else:
            # 如果事件循环未运行，则需要另外处理
            logger.info("事件循环未运行，将在适当时机启动主动聊天初始化任务")
            # 可以在这里添加稍后启动的逻辑，或者由外部调用 IdleManager.start_initialization_task()
    except RuntimeError:
